...
class SlideController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Slide']))
		{
			...
			{ 
				$data=CUploadedFile::getInstance($model,'file'); 
				$file=$data->name;
				$model->file=$file ;
			}
			$pathSlide=Yii::app()->basePath.'/../uploads/slide/';
			if($model->save()){
				$data->saveAs($pathSlide.$file);
				$image = Yii::app()->image->load($pathSlide.$file);
				$image->resize(640,320,image::WIDTH);
				$image->save();
